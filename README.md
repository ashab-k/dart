...
# 3. (Optional) Fill in API keys in .env
...
This fires 50 concurrent requests every 500ms for 60 seconds at the dummy server.

### What to Expect

1. **Server status goes DEGRADED** (orange) on the dashboard
2. **Anomaly detected** — dummy server auto-sends alert to DART backend
3. **Alert appears in live feed** with risk score (expect 40-95 for DDoS)
4. **Playbook executes** — `ddos-mitigation` blocks IP, sets rate limit, restarts server
...
|--------|----------|-------------|
| POST | `/api/alerts/ingest` | Ingest raw alert → full pipeline |
| GET | `/api/alerts/history` | All stored alerts |
